<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateEnrapportsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('enrapports', function (Blueprint $table) {
            $table->increments('id');
            $table->string('Identifiant')->unique();
            $table->string('num_ech')->nullable();
            $table->integer('produit_id')->nullable()->unsigned();
            $table->integer('client_id')->nullable()->unsigned();
            $table->integer('commercial_id')->nullable()->unsigned();
            $table->string('type')->nullable();
            $table->date('date_reception')->nullable();
            $table->date('date_impression')->nullable();
            $table->string('Interpretation')->nullable();
            $table->foreign('produit_id')->references('id')->on('produits');
            $table->foreign('client_id')->references('id')->on('clients');
            $table->foreign('commercial_id')->references('id')->on('commercials');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('enrapports');
    }
}
